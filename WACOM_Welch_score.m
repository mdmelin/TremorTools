...
for i = 1:length(task_index) %there are multiple trials for one type of task
    ...
    %figure; plot(f(f<20),p(f<20));

    
    PeakAmp  = max(p(f>frequencySearch(1) & f<frequencySearch(2))); % find peak amplitude and frequency of the resultant amplitude spectrum
    PeakFreq = f(find(p==PeakAmp));
    
    p_small = p(f>frequencySearch(1) & f<frequencySearch(2)); %restrict to freq search range
    f_small = f(f>frequencySearch(1) & f<frequencySearch(2));

    %%
    isHalfMax = ~(p < .707*PeakAmp);
    peakIndex = find(p==PeakAmp);
