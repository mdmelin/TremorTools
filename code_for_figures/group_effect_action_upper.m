...
FIGTITLE2 = 'Action tremor tasks with detected tremor only - upper body';
...
tasks = {'spiralBig_contra','lineBig_contra','spiralSmall_contra','lineSmall_contra'};

wacom_pre_tasks = []; wacom_post_tasks = []; wacom_pre_tremor_presence = [];
for i = 1:length(tasks)
    [wacom_pre, wacom_post, wacom_subjects, has_tremor] = allWACOMScores(tasks{i});
