function [pre_amps, post_amps, subjects] = allWACOMcores(task)
...
for i=1:length(subjects)
    ...
    if all(isnan(pre_amp))
        post_amp = NaN; post_freq = NaN; post_half = NaN;
    else
        post_amp = WACOM_Welch_score_at_freq(datadir, subjects{i},'post', taskname, 2, mean(pre_freq));
    end

    %[pre_amp,pre_freq,pre_half] = WACOM_Welch_score(datadir, subjects{i},'pre', taskname, 2);
    %[post_amp,post_freq,post_half] = WACOM_Welch_score(datadir, subjects{i},'post', taskname, 2);

    pre_amps = [pre_amps, nanmean(pre_amp)];
    post_amps = [post_amps, nanmean(post_amp)];

    pre_freqs = [pre_freqs, nanmean(pre_freq)];
    %post_freqs = [post_freqs, mean(post_freq)];

    pre_halfs = [pre_halfs, nanmean(pre_half)];
